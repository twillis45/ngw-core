...
- **`catchlight_reliability == "blocked"` is NOT a glasses detector.** Phase 3B / Phase 3C use an "unresolvable-catchlight-signal" detector that fires when the resolver produces a low-confidence demoted winner with many catchlights. This catches the glasses test case as one shape of contamination but also fires on mirror selfies, heavy reflective contamination, etc. Naming is honest throughout: variable `catchlight_unreliable`, reason `low_confidence_demoted_winner_with_many_catchlights`, test class `TestCatchlightUnreliabilityDetector`. A true glasses CV detector remains future work.
- **Phase 3D Workstream C — deferred (not landed).** Phase 3D had explicit narrow authorization to add ONE bounded eye-region extractor in `engine/cue_extraction.py`. After investigation the workstream was stopped without a commit because (1) `cue_extraction.py` does not have per-eye iris landmark data accessible (MediaPipe iris landmarks live in `vision_passes.py`, out of scope); (2) per-catchlight `elevation_ratio` data (which IS iris-relative) was empirically tested in Phase 3C and found NOT discriminative — clean butterfly portraits show similar anomalous low-elevation catchlight signatures from skin specular; (3) a face_box-approximated eye region is too coarse for a truthful iris-relative spatial-signature detector; (4) robust threshold validation requires multi-image CV iteration cycles outside one bounded commit. Per Phase 3D's explicit fallback clause (`If truthful success is not possible in scope: do not fake it; preserve the honest renamed proxy; stop cleanly`), the existing Phase 3B/C `catchlight_unreliable` detector is preserved unchanged. A real glasses CV detector remains Phase 3E+ work and will require either (a) per-eye iris landmark data plumbed into cue_extraction, or (b) a new eye-region pass alongside cue_extraction, or (c) image-bytes plumbing at the routing stage with a dedicated detector. The Phase 3C honest-renaming decision (option 2) stands.
...
- **CLA→BND borderlines (Phase 3C Workstream D / Phase 3D Workstream B).**
    - **`white_seamless_catalog`** — RESOLVED in Phase 3D/B. Phase 3D added a `background_class` axis on `ComplexityProfile` derived from existing `cue_report.background_illumination` (no new CV). When `background_class == "uniform_seamless"` (pattern=even AND brightness_relative=darker — the catalog/product signature), the long-term BOUNDED predicate is suppressed and the case routes CLASSICAL. Verified to NOT affect rihanna BOUNDED (gradient bg) or `bounded_loop_vs_short_jewelry_t1` (uniform_neutral bg, brightness=similar). Catalog photography is intrinsically classical — pose-relative classifier disagreement on uniform-seamless setups is engine noise, not real ambiguity.
    - **`window_soft_side`** — STILL ACCEPTED AMBIGUITY. Background_class is "environmental"; the uniform_seamless suppression doesn't apply. Engine signals remain byte-identical to confirmed-BOUNDED rihanna and jewelry BND hits at the credibility-list level. Differential photographer tagging is judgment-call variability. Documented as a photographer-re-review candidate if a future curation pass revisits it. No predicate change for this case.
- **Phase 3D/A demotion-forgiveness.** When a demoted candidate has independent structural support (a raw-signal classifier OTHER than the candidate's own source voted for the same pattern) AND no relevant paradox is active, the demotion is forgiven: trust multiplier is lifted from `SOURCE_TRUST_DEMOTED` to `SOURCE_TRUST_CLEAN`, demotion-event contradictions for the candidate are skipped (no double-counting), and a forgiveness-aware "credibility overrules resolver" branch fires when the forgiven alt disagrees with the resolver primary (preserves rihanna-style confirmed BOUNDED hits, recovers `bounded_loop_vs_rembrandt_bw_t1`). Forgiveness is BLOCKED by active paradox flags (`flat_bilateral`, `bilateral_deep_shadow`, `spurious_triangle`, `highlight_shadow_discord`, or catchlight-shadow paradox). The remaining BOUNDED miss (`bounded_butterfly_vs_clamshell_beauty`) hits the paradox-block correctly — recovering it would require either resolving the paradox upstream or better classifier alternates, both outside Phase 3D scope.
