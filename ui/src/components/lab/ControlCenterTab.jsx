...
const ALERT_RULES = [
  {
    ...
    desc: 'Alert ≥ 15%, warn ≥ 8%. VLM failures degrade every analysis.',
    threshold: 0.15,
    getValue: m => m?.error_rate ?? null,
    evaluate: (v, t) => v === null ? 'unknown' : v >= t ? 'alert' : v >= 0.08 ? 'warn' : 'ok',
    format: v => v === null ? '—' : `${(v * 100).toFixed(1)}%`,
  },
  {
    id: 'vlm_no_calls',
    label: 'VLM Call Volume',
    desc: 'Warn < 5 calls/24h, alert if zero. Catches integration breakage.',
    threshold: 0,
    getValue: m => m?.total ?? null,
    evaluate: (v, t) => v === null ? 'unknown' : v === 0 ? 'alert' : v < 5 ? 'warn' : 'ok',
    format: v => v === null ? '—' : `${v} calls`,
  },
  {
    id: 'vlm_p95_latency',
    label: 'VLM p95 Latency',
    desc: 'Alert ≥ 6s, warn ≥ 3.6s. Provider degradation hurts UX.',
    threshold: 6000,
    getValue: m => m?.p95_latency_ms ?? null,
    evaluate: (v, t) => v === null ? 'unknown' : v >= t ? 'alert' : v >= 3600 ? 'warn' : 'ok',
    format: v => v === null ? '—' : `${(v / 1000).toFixed(1)}s`,
  },
  {
    id: 'vlm_avg_latency',
    label: 'VLM Avg Latency',
    desc: 'Alert ≥ 3s, warn ≥ 2s. Early degradation signal.',
    threshold: 3000,
    getValue: m => m?.avg_latency_ms ?? null,
    evaluate: (v, t) => v === null ? 'unknown' : v >= t ? 'alert' : v >= 2000 ? 'warn' : 'ok',
    format: v => v === null ? '—' : `${(v / 1000).toFixed(1)}s`,
  },
];
